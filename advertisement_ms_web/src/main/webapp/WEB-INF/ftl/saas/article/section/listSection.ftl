...
$(document).ready(function(){
	dataTableConfig.ajax = "${rc.contextPath}/admin/article/${functionName}/query.do?articleId=${articleId}";
	dataTableConfig.columns = [{
	        ...
	        "render": function ( data, type, full, meta ) {
	            return '<a target="_blank" href="${rc.contextPath}/admin/article/${functionName}/detail.do?${functionId}='+data+'"  class="btn btn-blue">详情</a>';
	        }
	    }, {
	    	"data": "${functionId}",
	        "render": function ( data, type, full, meta ) {
	            return '<a target="_blank" href="${rc.contextPath}/admin/article/${functionName}/updateWindow.do?${functionId}='+data+'"  class="btn btn-cyan">修改</a>';
	        }
	    }, {
	    	"data": "${functionId}",
	        "render": function ( data, type, full, meta ) {
	        	var deleteUrl = '${rc.contextPath}/admin/article/${functionName}/delete.do?status=-1&${functionId}='+data;
	...
	<div class="tip" id="pop-tip">
		...
		<div id="selects">
			 <select onChange="getValue(this)" name="select">
			 	<option value="${rc.contextPath}/admin/article/section/list.do?articleId=${articleId}" selected>请选择版本</option> 
            	<option value="${rc.contextPath}/admin/article/section/insertWindow.do?articleId=${articleId}">简单版</option>
            	<option value="${rc.contextPath}/admin/article/section/complexWindow.do?articleId=${articleId}">复杂版</option>	
             </select>
		</div>
