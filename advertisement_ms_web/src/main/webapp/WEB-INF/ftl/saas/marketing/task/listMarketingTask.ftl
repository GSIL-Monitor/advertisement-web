...
<script>
	$(document).ready(function(){
		...
		dataTableConfig.columns = [{
		    	...
		    	"data": "${functionId}",
		        "render": function ( data, type, full, meta ) {
		        	var updateUrl = '${rc.contextPath}/admin/${functionName}/execute.do?${functionId}='+data;
		            return '<a href="###" class="btn btn-green" onclick="confirmSend(\''+updateUrl+'\', \'确定执行任务吗\');">执行任务</a>';
		        }
		    }, {
	    		"data": "${functionId}",
	        	"render": function ( data, type, full, meta ) {
		        ...
		        "render": function ( data, type, full, meta ) {
		        	 if (data.downloadUrl != null && data.downloadUrl != '') {            	
		            	return '<a  href="'+data.downloadUrl+'" class="btn btn-blue" target="_blank">下载</a>';		            
		           	  } else {
		           		return  '<a class="btn btn-cyan">f5刷新页面</a>'
		              }    
		        }   
		    }, {
	    		"data": "${functionId}",
		...
		$('#queryButton').on('click', function(){
			...
			var newUrl="${rc.contextPath}/admin/${functionName}/query.do?productName="+encodeURI(encodeURI(searchText));
...
<div id="content">
	...
	<div class="container-fluid">
		<hr>
			<div class="row-fluid">
				<span style="float:right;margin:3px 8px 10px 0"><a href="${rc.contextPath}/admin/${functionName}/insertWindow.do" target="_blank"><button class="btn btn-cyan">添加营销任务</button></a></span>
