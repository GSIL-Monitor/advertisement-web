...
$(document).ready(function(){
	...
	dataTableConfig.columns = [{
	    	...
	    	"data": "${functionId}",
	        "render": function ( data, type, full, meta ) {
	        	var deleteUrl = '${rc.contextPath}/admin/${functionName}/delete.do?productId=${productId}&status=-1&${functionId}='+data;
...
<div id="selects">
	<select onChange="getValue(this)" name="select">
		<option value="${rc.contextPath}/admin/product/article/list.do?productId=${productId}" selected>请选择版本</option>
		<option value="${rc.contextPath}/admin/product/article/simpleArticle.do?productId=${productId}">简单版</option>
		<option value="${rc.contextPath}/admin/product/article/insertWindow.do?productId=${productId}">复杂版</option>
	</select>
</div>
