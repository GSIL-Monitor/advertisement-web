...
$(document).ready(function(){
	dataTableConfig.ajax = "${rc.contextPath}/admin/${functionName}/query.do?channel=${key}&activityId=${activityId}";
	...
	$('#queryButton').on('click', function(){
		...
		var newUrl="${rc.contextPath}/admin/${functionName}/query.do?channel=${key}&activityId=${activityId}&action="+encodeURI(encodeURI(searchText));
...
<div id="content">
  ...
  <div class="container-fluid">
    <hr>
    <div class="row-fluid">
    <span style="float:right;margin:3px 8px 10px 0"><a href="${rc.contextPath}/admin/channel/${functionName}/insertWindow.do?channel=${key}&activityId=${activityId}" target="_blank"><button class="btn btn-yellow">添加${functionTitle}</button></a></span>
        <div class="widget-box">
          <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
            <h5>数据表格</h5>
            <div style="float:right;margin:3px 8px 10px 0">	            
				<input type="text" name="search" id="search" value="action查询"/>
