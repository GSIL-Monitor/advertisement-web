...
public class AdvertisementStrategyDaoImpl extends BaseDaoImpl implements AdvertisementStrategyDao {
	...
	public List<AdvertisementStrategy> selectAdvertisementStrategy(AdvertisementStrategy advertisementStrategy,
		...
		return getSqlSession().selectList("advertisementStrategy.selectAdvertisementStrategy", advertisementStrategy,
				pageBounds);
	}

	@Override
	public List<AdvertisementStrategy> selectAdvertisementStrategyByIds(List<Long> advertisementStrategyIds) {
		if (advertisementStrategyIds == null || advertisementStrategyIds.size() == 0) {
			return new ArrayList<AdvertisementStrategy>();
		}
		return getSqlSession().selectList("advertisementStrategy.selectAdvertisementStrategyByIds",
				advertisementStrategyIds);
	}
	...
	public List<AdvertisementStrategy> selectAdvertisementStrategyByFunctionIds(List<Long> functionIds) {
		...
		return getSqlSession()
				.selectList("advertisementStrategy.selectAdvertisementStrategyByFunctionIds", functionIds);
	}
