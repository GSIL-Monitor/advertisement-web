...
import com.yuanshanbao.ad.statistics.dao.AdvertisementStatisticsDao;
import com.yuanshanbao.ad.statistics.model.AdvertisementStatistics;
import com.yuanshanbao.ad.statistics.model.AdvertisementStatisticsType;
...
public class AdvertisementStatisticsServiceImpl implements AdvertisementStatisticsService {
	...
	private void setUv(String date, String channel, List<AdvertisementStatistics> resultList, Long advertisementId) {
		...
		adStatistics.setWelfareCount(getUv(date, channel, AdvertisementPosition.WELFARE, advertisementId));
		adStatistics.setBannerCount(getUv(date, channel, AdvertisementPosition.BANNER, advertisementId));
		adStatistics.setTagsCount(getUv(date, channel, AdvertisementPosition.TAGS, advertisementId));
		adStatistics.setDownloadCount(getUv(date, channel, AdvertisementPosition.DOWNLOAD, advertisementId));
		adStatistics.setTotal(calculateTotal(adStatistics));
	...
	private void setPv(String date, String channel, List<AdvertisementStatistics> resultList, Long advertisementId) {
		...
		adStatistics.setWelfareCount(getPv(date, channel, AdvertisementPosition.WELFARE, advertisementId));
		adStatistics.setBannerCount(getPv(date, channel, AdvertisementPosition.BANNER, advertisementId));
		adStatistics.setTagsCount(getPv(date, channel, AdvertisementPosition.TAGS, advertisementId));
		adStatistics.setDownloadCount(getPv(date, channel, AdvertisementPosition.DOWNLOAD, advertisementId));
		adStatistics.setTotal(calculateTotal(adStatistics));
		resultList.add(adStatistics);
	}
	...
	public List<SuccessPageClick> selectSuccessPageClicks(int dateDiff, boolean fromDB) {
		...
		for (String channel : channelList) {
			...
			String successUvStr = (String) redisCacheService.get(RedisConstant.getResultPageUVCountKey(
					AdvertisementPosition.WELFARE, date, channel));
			int count = 0;
			...
			String failUvStr = (String) redisCacheService.get(RedisConstant.getResultPageUVCountKey(
					AdvertisementPosition.BANNER, date, channel));
			count = 0;
