package com.yuanshanbao.paginator.interceptor;
...
import com.yuanshanbao.paginator.dialect.Dialect;
import com.yuanshanbao.paginator.domain.PageBounds;
import com.yuanshanbao.paginator.domain.PageList;
import com.yuanshanbao.paginator.domain.Paginator;
import com.yuanshanbao.paginator.support.PropertiesHelper;
import com.yuanshanbao.paginator.support.SQLHelp;
...
public class PaginationInterceptor implements Interceptor {
			...
			if (pageBounds.isContainsTotalCount()) {
				countTask = new Callable() {
					public Object call() throws Exception {
						BoundSql boundSql = ((MappedStatement) queryArgs[MAPPED_STATEMENT_INDEX])
						...
						if (cache != null && ms.isUseCache()) {
							CacheKey cacheKey = executor.createCacheKey(ms, parameter, new PageBounds(),
									copyFromBoundSql(ms, boundSql, sql));
							count = (Integer) cache.getObject(cacheKey);
							if (count == null) {
								count = SQLHelp.getCount(sql, ms, parameter, boundSql, dialect);
	...
	public void setDialect(Dialect dialect) {
		LoggerUtil.debug("dialectClass: " + dialect.getClass().getName());
		this.dialect = dialect;
	}

	public void setAsyncTotalCount(boolean asyncTotalCount) {
		LoggerUtil.debug("asyncTotalCount: " + asyncTotalCount);
	...
	public void setPoolMaxSize(int poolMaxSize) {
		...
		if (poolMaxSize > 0) {
			LoggerUtil.debug("poolMaxSize: " + poolMaxSize);
