...
public class ZXingCode {
	...
	public static void main(String[] args) throws WriterException {
		File logoFile = new File("D://r.png");
		File QrCodeFile = new File("D://4953774.jpeg");
		...
		try {
			...
			if (StringUtils.isNotEmpty(note)) {
				...
				if (strWidth > 399) {
					...
					outg2.drawString(note2, 200 - strWidth2 / 2, outImage.getHeight()
							+ (outImage2.getHeight() - outImage.getHeight()) / 2 + 5);
					outg2.dispose();
					outImage2.flush();
					outImage = outImage2;
				} else {
	 ...
	 *            头像路径
	 */
	public static String getLogoQRCode(String content, String avatar) {
		try {
			MultiFormatWriter multiFormatWriter = new MultiFormatWriter();
			// 参数顺序分别为：编码内容，编码类型，生成图片宽度，生成图片高度，设置参数
			...
			if (Objects.nonNull(avatar)) {
				...
				// 开始绘制logo图片
				// g.drawImage(logo, 60, 60, 45, 45, null);
				g.drawImage(logo, width * 2 / 5, height * 2 / 5, width * 2 / 10, height * 2 / 10, null);
				g.dispose();
				logo.flush();
			}
			...
			String lognCode = UploadUtils.uploadBytes(is, is.available(), "test/image/avatar" + System.nanoTime()
