...
import com.yuanshanbao.common.constant.BaseConstant;
...
public class SMSUtil {
	 ...
	 * @return 验证结果
	 */
	public static boolean veryCheckCode(String mobile, String checkCode, String msgprop) {
		boolean result = false;
		String wirelessUrl = new StringBuffer(SMS_CHECKCODE_URL).append("phone=").append(mobile)
				.append("&code=").append(checkCode).append("&msgprop=").append(msgprop).toString();

		LogUtil.debug(wirelessUrl, LogUtil.LOG_BASIC_FRAMEWORK);

		try {
			String resultstr = HttpUtil.sendGetRequest(wirelessUrl);
			String[] tmp = resultstr.split("\n");
			return tmp != null && tmp.length > 1 && "1".equalsIgnoreCase(tmp[0].trim());
		} catch (Exception e) {
			// throw exception.
	 ...
	 * @return 发送结果
	 */
	public static String sendMessage(String mobile, String opcode, String msgprop) throws Exception {
		...
		try {
			...
			String url = new StringBuffer(BaseConstant.getValue("smsSendURL")).append("?phone=")
					.append(mobile).append("&frmphone=").append(mobile).append("&msgprop=")
					.append(msgprop).append("&message=").append(msg).append("&corpinfo=1")
					.toString();
