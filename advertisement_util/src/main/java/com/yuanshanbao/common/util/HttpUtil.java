...
public class HttpUtil {
	...
	public static String sendRequest(String url, int connectTimeout, int readTimeout, String charset,
		...
		try {
			...
			// logger.error("", e);
			LoggerUtil.alarmInfoStrategy10m10("[通用模块][紧急][网络IO异常 : " + e.getMessage() + ", 调用堆栈 : "
					+ locateInvokerStack(getErrorStack(e)));
	...
	public static String forward(HttpServletRequest request, String url, int connectTimeout, int readTimeout,
		...
		url = url + param;
		return sendRequest(url, connectTimeout, readTimeout, charset, returnSingle);
	}

	public static String forward(HttpServletRequest request, String url) {
		return forward(request, url, DEFAULT_CONNECT_TIME_OUT, DEFAULT_READ_TIME_OUT, DEFAULT_CHARSET, false);
	}
	...
	public static String sendPostRequest(String url, String content, String charset, Map<String, String> header) {
		return sendPostRequest(url, content, charset, DEFAULT_CONNECT_TIME_OUT, DEFAULT_READ_TIME_OUT, header);
	}

	public static String sendPostRequest(String url, String content, String charset) {
		return sendPostRequest(url, content, charset, DEFAULT_CONNECT_TIME_OUT, DEFAULT_READ_TIME_OUT, null);
	}
	...
	public static String sendPostRequestWithHeader(String url, String content, String charset, int connectTimeout,
			int readTimeout, boolean needCompress, Map<String, String> header) throws RemoteInvocationFailureException {
		BufferedReader in = null;
		...
		try {
			...
			if (header != null) {
				for (String key : httpConn.getHeaderFields().keySet()) {
					header.put(key, httpConn.getHeaderField(key));
				}
			}
			return result;
		} catch (IOException e) {
	...
	public static byte[] sendGetRequestForInputStream(String url, int connectTimeout, int readTimeout)
	...
	public static byte[] sendGetRequestAndGetBytes(String url) throws MalformedURLException, IOException, AppException {
		try {
			return sendGetRequestForInputStream(url, DEFAULT_CONNECT_TIME_OUT, DEFAULT_READ_TIME_OUT);
		} catch (Exception e) {
			LoggerUtil.error("send request error: " + url, e);
			throw e;
		}
	}

	public static InputStream sendRequestForInputStream(String url, String content, String charset, int connectTimeout,
		...
		try {
			...
			// logger.debug("请求发送地址:" + url);
			// logger.debug("参数:" + content);
			return postConnect(httpConn, content, charset, needCompress);
		} catch (Exception e) {
			// logger.error("", e);
			throw new RemoteInvocationFailureException("网络IO异常[" + url + "]", e);
		} finally {
	...
	public static String sendPostRequest(String url, String content, String charset, boolean needCompress)
			throws RemoteInvocationFailureException {
		return sendPostRequestWithHeader(url, content, charset, DEFAULT_CONNECT_TIME_OUT, DEFAULT_READ_TIME_OUT,
				needCompress, new HashMap<String, String>());
	}
	 ...
	 */
	public static String sendGetRequest(String url) throws MalformedURLException, IOException, AppException {
		return sendGetRequest(url, DEFAULT_CONNECT_TIME_OUT, DEFAULT_READ_TIME_OUT, null, null);
	}

	public static String sendGetRequest(String url, Map<String, String> header) throws MalformedURLException,
			IOException, AppException {
		return sendGetRequest(url, DEFAULT_CONNECT_TIME_OUT, DEFAULT_READ_TIME_OUT, null, header);
	}
	 ...
	 * @throws AppException
	 */
	public static String sendGetRequest(String url, SSLContext sslContext) throws MalformedURLException, IOException,
			AppException {
		return sendGetRequest(url, DEFAULT_CONNECT_TIME_OUT, DEFAULT_READ_TIME_OUT, sslContext, null);
	}
	 ...
	 * @throws AppException
	 */
	public static String sendGetRequest(String url, int connectTimeout, int readTimeout, SSLContext sslContext,
			Map<String, String> header) throws MalformedURLException, IOException, AppException {
		InputStream is = null;
		HttpURLConnection conn = null;
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		BufferedInputStream bis = null;
	...
	public static byte[] sendPostRequestForBytes(String url, String content, String charset)
