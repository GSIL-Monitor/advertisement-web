...
public class XmlUtil {
	...
	public static Element mapToXmlElement(Map input, Element root, String rootName) {
		...
		while (itrInput.hasNext()) {
			...
			if (value instanceof Map) {
				...
				if ((valueToString.indexOf("<") >= 0 && valueToString.indexOf("</") >= 0)
	...
	public static Map xmlToMap(String xml) {
		try {
			...
			LoggerUtil.error("[XmlUtil.xmlToMap]", e);
		}
		return new HashMap();
	}
	...
	private final static Map<Class<?>, Class<?>> SIMPLE_TYPE = new HashMap<Class<?>, Class<?>>(16);
	...
	public static boolean isSimpleElementType(Class<?> clazz) {
		return SIMPLE_TYPE.keySet().contains(clazz) || SIMPLE_TYPE.values().contains(clazz);
	}
	 ...
	 */
	public static void beanToXmlElement(final Element parentElement, final Object bean,
			final List<String> noToProcessedFieldList) {
		if (Collection.class.isAssignableFrom(bean.getClass()) || bean.getClass().isArray()) {
			throw new UnsupportedOperationException("Collection");
	 ...
	 */
	public static Map objectToMap(Object object) throws SecurityException, NoSuchMethodException,
			IllegalArgumentException, IllegalAccessException, InvocationTargetException, DocumentException {
